...
\subsection{Implementation: Formal Ethical Constraints}
...
To ensure clarity, the following logical predicates define the core parameters of our CitySwift model:
\begin{itemize}
  \item \textbf{Reference Baseline}: The ideal service level (buses per hour) mandated by the franchise agreement. In this model, the baseline represents the "100\% target" from which ethical deviations are measured.
  \item \textbf{Ethical Minimum (Floor)}: Defined by Case 1 and Case 4, these are the absolute service floors (e.g., 50\% for hospital routes, 60\% for urban zones) that the AI is strictly forbidden from crossing, even when optimizing for efficiency.
  \item \textbf{Passenger Demand}: The predicted volume of commuters for a route, used to drive utilitarian optimization across the remaining fleet capacity.
  \item \textbf{Route Criticality}: A binary flag identifying essential infrastructure (hospitals, schools) that triggers the highest level of deontological protection.
\end{itemize}

\subsection{Formal Ethical Constraints (8 Cases)}

Using the Z3 library, we implemented the following eight cases, each mapping to a specific stakeholder and ethical principle:
...
# 7. Ethical Constraints: Hospital routes >= 50% baseline
for r in routes:
    optimizer.add(z3.Implies(is_critical(r), 
                            frequency(r) >= baseline_freq(r) / 2))
\end{lstlisting}
...
\begin{lstlisting}[style=py]
# Virtues are weighted; they can be overridden in extreme dilemmas.
...
for z in zones:
    zone_baseline = z3.Sum([z3.If(route_zone(r) == z, baseline_freq(r), 0) for r in routes])
    zone_current = z3.Sum([z3.If(route_zone(r) == z, frequency(r), 0) for r in routes])
...
driver_hours_limit = 20 * 8 # num_drivers * limit
optimizer.add(z3.ToReal(z3.Sum([frequency(r) for r in routes])) * 1.2 <= driver_hours_limit)
\end{lstlisting}

\subsection{Case 7: Predictable Scheduling vs. Dynamic Efficiency (Reliability)}
While dynamic headway adjustment eliminates "bus bunching," it can frustrate passengers who rely on a consistent timetable. This constraint limits real-time deviations to a +/- 15-minute window from the public-facing schedule, prioritizing reliability over pure mathematical efficiency.
\begin{lstlisting}[style=py]
# Dynamic adjustments cannot exceed 15 mins from public timetable
for r in routes:
    optimizer.add(z3.Abs(opt_headway(r) - scheduled_h(r)) <= 15)
\end{lstlisting}

\subsection{Case 8: GDPR Anonymization vs. Predictive Precision (Privacy)}
To protect passenger data rights, the system is logically prevented from using individual-level travel histories for demand forecasting unless explicit user consent has been registered, as mandated by GDPR frameworks.
\begin{lstlisting}[style=py]
...
optimizer.add(z3.Implies(z3.Not(consent_given), 
                        z3.Not(use_individual_data)))
optimizer.add(consent_given == False) # Scenario
\end{lstlisting}
...
The primary conflict arose from the tension between Bus Éireann's value of maximized network throughput (Utilitarianism) and the commuters' value of equitable access. A purely efficient algorithm would mathematically favor high-density urban corridors, risking the creation of "transit deserts" in rural areas. We revisited this by implementing Case 1 (Minimum Service Baseline) and Case 4 (Transit Desert Prevention) as hard constraints rather than optimization goals. This ensures that marginalized communities benefit from guaranteed mobility, even at the cost of marginal network-wide efficiency gains.

\paragraph{Ecological Stewardship vs. Virtue of Care}
While both CitySwift and residents value carbon reduction (Virtue Ethics), we identified a conflict during extreme weather events. A strict anti-idling protocol, though virtuous for the environment, violates the virtue of care for vulnerable passengers in sub-zero temperatures. The design was changed to include Case 3 (Extreme Weather Prioritization), which introduces a temperature-triggered logic override. During storms or freezes, the system prioritizes human safety and comfort over carbon-neutral optimization, benefiting elderly and transit-dependent populations.

\paragraph{Data-Driven Accuracy vs. Worker Safety}
To achieve high-fidelity forecasting (Technical Value), the system requires granular telemetry of bus movements. However, this creates a conflict with driver privacy and the legal duty to manage fatigue. We addressed this by implementing Case 6 (Driver Fatigue Management), which acts as a "safety brake" on the optimization process. This ensures that the drive for predictive accuracy never culminates in a driver workload that exceeds legal safety limits, benefiting both the drivers (fairness) and the general public (safety).

\paragraph{Algorithmic Transparency vs. Punitive Accuracy}
The Utilitarian drive for predictive accuracy often relies on "black-box" sub-symbolic models. This creates a Deontological conflict regarding the right to transparent due process if an operator is penalized based on an opaque classification. We revisited the design by implementing Case 2 (Penalty Categorization Confidence), which enforces a 95\% confidence threshold and maintains a fallback to human-readable symbolic logic. This protects operators from arbitrary algorithmic penalties and ensures that judicial/contractual decisions remain auditable.

\paragraph{Predictable Reliability vs. Dynamic Throughput}
While the system aims to dynamically eliminate "bus bunching" through real-time headway adjustment, this conflicts with the passenger’s need for a stable and predictable timetable. A passenger planning their journey around a fixed time (\textit{predictability}) may find real-time variations counter-productive, even if they improve network-wide flow (\textit{throughput}). We addressed this in Case 7 by constraining algorithmic autonomy to a +/- 15-minute deviation window, prioritizing passenger reliability and institutional trust over pure mathematical optimization.

\paragraph{Data Privacy vs. Predictive Precision}
The system’s predictive accuracy (Technical Value) depends on high-dimensional datasets, including ticketing and origin-destination flows. However, this creates a Deontological conflict with the passenger's right to privacy and GDPR-mandated data minimization. A purely optimized system would use personalized travel histories to anticipate individual needs. We revisited this in Case 8 by formalizing a consent-based logical gate: unless explicit user consent is registered, the system is strictly limited to using aggregated, anonymized zone data, prioritizing individual privacy over marginal gains in predictive precision.

\section{Evidence}
Validation of the functional and ethical goals was achieved by executing the \texttt{cityswift\_z3\_implementation.py} script. The solver leverages First-Order Logic (FOL) predicates and soft-constraint optimization to reconcile all 8 ethical cases. The successful ethical reconciliation is shown in the terminal output below, following the formal style of the module's logical proofs.

\begin{lstlisting}[style=mystyle, caption={Terminal Output of CitySwift Ethical Logic Engine}]
Executing CitySwift Ethical Logic Engine...
Scenario Parameters:
- Total Fleet Limit: 60 buses
- Driver Capacity: 20 drivers (8.0 hrs/shift)
- Input Mock Routes [ID | Zone | Base | Demand | Critical]:
  R0  | Z0   | 4    | 100    | Yes
  R1  | Z0   | 6    | 80     | No
  R2  | Z1   | 4    | 120    | Yes
  R3  | Z1   | 8    | 200    | No
...
Route ID  | Zone     | Allocated (Base)   | Demand     | Status
-------------------------------------------------------------
Route 0    | Z0       | 6  (4 ) buses/hr | 100    pax |  [CRITICAL]
Route 1    | Z0       | 0  (6 ) buses/hr | 80     pax | 
Route 2    | Z1       | 2  (4 ) buses/hr | 120    pax |  [CRITICAL]
Route 3    | Z1       | 6  (8 ) buses/hr | 200    pax | 
Route 4    | Z2       | 6  (5 ) buses/hr | 50     pax | 
Route 5    | Z2       | 0  (5 ) buses/hr | 30     pax | 
Route 6    | Z3       | 34 (10) buses/hr | 300    pax | 
Route 7    | Z3       | 0  (4 ) buses/hr | 20     pax | 
Route 8    | Z4       | 6  (6 ) buses/hr | 150    pax | 
Route 9    | Z4       | 0  (4 ) buses/hr | 10     pax | 

Zone Coverage Summary (Distributive Justice):
---------------------------------------------
Z0      : 6.0/10.0 buses/hr (60.0%) - [SATISFIED]
Z1      : 8.0/12.0 buses/hr (66.7%) - [SATISFIED]
Z2      : 6.0/10.0 buses/hr (60.0%) - [SATISFIED]
Z3      : 34.0/14.0 buses/hr (242.9%) - [SATISFIED]
Z4      : 6.0/10.0 buses/hr (60.0%) - [SATISFIED]

Ethical Governance Status:
- [Case 1] Hospital/School Protection: True
- [Case 4] Transit Desert Prevention: Minimum 60% coverage enforced per zone.
- [Case 5] Utilitarian Optimization: Efficiency maximized (Score: 13440)
- [Case 6] Driver Safety: Avg workload 3.6 hrs/shift (Limit: 8.0).

Operational Metadata:
- Weather Care Mode: True
- Audit Status: False (Confidence 23/25)
- GDPR Status: Anonymized
\end{lstlisting}

\paragraph{Logic Analysis}
The expanded solver output provides a comprehensive audit trail of the ethical reconciliation process. By explicitly displaying the \textbf{Zone Coverage Summary}, the engine proves that distributive justice (Case 4) is mathematically satisfied, with every urban zone receiving at least 60\% of its \textbf{Reference Baseline}. Furthermore, the \textbf{Ethical Governance Status} section confirms that both deontological minimums (Case 1) and utilitarian optimization (Case 5) are active and successfully reconciled. This level of transparency ensures that the system's decisions are explainable, verifiable, and ethically grounded for all stakeholders.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{z3_success_output.png}
  \caption{Output showing Z3 successfully reconciling 8 ethical constraints using FOL and Soft-Constraints.}
\end{figure}
