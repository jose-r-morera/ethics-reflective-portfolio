...
for i in range(num_routes):
    ...
    optimizer.add(optimized_h > 0) # Headway must be positive
\end{lstlisting}
...
\section{Evidence}
Validation of the functional and ethical goals was achieved by executing the \texttt{cityswift\_z3\_implementation.py} script. The solver successfully identifies configurations that satisfy all 8 ethical constraints simultaneously, as shown in the terminal output below. In scenarios where ethical requirements (e.g., driver safety or minimum zone capacity) are impossible to meet with the available fleet, the system flags "UNSAT," preventing an unethical deployment.

\begin{lstlisting}[style=mystyle, caption={Terminal Output of CitySwift Ethical Solver (8 Cases)}]
Running CitySwift Ethical Constraint Solver (8 Cases)...

--- RESULTS ---
Route 0 (Zone 0, Critical: True): Assigned Freq = 6
Route 1 (Zone 0, Critical: False): Assigned Freq = 0
Route 2 (Zone 1, Critical: True): Assigned Freq = 2
Route 3 (Zone 1, Critical: False): Assigned Freq = 6
Route 4 (Zone 2, Critical: False): Assigned Freq = 6
Route 5 (Zone 2, Critical: False): Assigned Freq = 0
Route 6 (Zone 3, Critical: False): Assigned Freq = 34
Route 7 (Zone 3, Critical: False): Assigned Freq = 0
Route 8 (Zone 4, Critical: False): Assigned Freq = 6
Route 9 (Zone 4, Critical: False): Assigned Freq = 0

Incident Operator Controlled: False
Prioritize Passenger Care (Weather): True
Use Individual Travel History (GDPR): False

Total Fleet Used: 60 / 60
Ethical Constraints Reconciled: YES (8/8 Cases)
\end{lstlisting}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{z3_success_output.png}
  \caption{Output showing Z3 successfully reconciling 8 ethical constraints with network demand.}
\end{figure}
