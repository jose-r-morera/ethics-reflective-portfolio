...
\begin{lstlisting}[style=py]
# Every zone must retain at least 60% of baseline total capacity
for z in zones:
    routes_in_zone = [r for r, r_zone, b, d, c, sh in mock_data if r_base_zone(r_zone) == str(z)]
    z_baseline = sum([b for i, r_zone, b, d, c, sh in mock_data if r_base_zone(r_zone) == str(z)])
    zone_sum = z3.Sum([frequency(r) for r in routes_in_zone])
    optimizer.add(z3.ToReal(zone_sum) >= z3.ToReal(z_baseline) * 0.6)
\end{lstlisting}
...
\section{Evidence}
Validation of the functional and ethical goals was achieved by executing the \texttt{cityswift\_z3\_implementation.py} script. The solver leverages First-Order Logic (FOL) predicates and soft-constraint optimization to reconcile all 8 ethical cases. The successful ethical reconciliation, including the full scenario context and distributive justice audit, is shown in the actual terminal screenshot below (Figure \ref{fig:z3_output}).

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\textwidth]{z3_success_output.png}
  \caption{Terminal output of the CitySwift Ethical Logic Engine, showing successful reconciliation of Deontological, Utilitarian, and Distributive constraints.}
  \label{fig:z3_output}
\end{figure}

\paragraph{Logic Analysis}
The terminal screenshot provides a comprehensive audit trail of the ethical reconciliation process. By explicitly displaying the \textbf{Zone Coverage Summary}, the engine proves that distributive justice (Case 4) is mathematically satisfied, with every urban zone receiving at least 60\% of its \textbf{Reference Baseline}. Furthermore, the \textbf{Ethical Governance Status} section confirms that both deontological minimums (Case 1) and utilitarian optimization (Case 5) are active and successfully reconciled. This level of transparency ensures that the system's decisions are explainable, verifiable, and ethically grounded for all stakeholders.
